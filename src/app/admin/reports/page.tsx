...
export default function AdminReportsPage() {
  ...
  const totals = data?.rows?.reduce(
    (acc, r) => ({ impressions: acc.impressions + (r.Impressions || 0), revenue: acc.revenue + (r.Revenue || 0) }),
  ...
  const chartData = (data?.rows || [])
    .filter((r) => r.Date)
    .map((r) => ({ date: r.Date!, revenue: r.Revenue || 0, impressions: r.Impressions || 0 }))
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Network Reports</h1>
          <p className="text-gray-500 text-sm mt-1">Network-wide ad performance</p>
        </div>
      ...
      {data?.meta?.degraded && (
        <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded-lg text-sm">
          <AlertTriangle size={16} /> Data may be incomplete. Complete through: {data.meta.dataCompleteThrough?.slice(0, 10)}
        </div>
      )}
      ...
      {dimension === "Date" && chartData.length > 0 && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          <Card>
            <CardHeader><CardTitle>Impressions trend</CardTitle></CardHeader>
              ...
              <table className="w-full text-sm">
                  ...
                  {data.rows.map((row, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-gray-50">
                        <td className="px-4 py-3 font-medium text-gray-900">{row.Date || row.Country || "—"}</td>
                        ...
                        <td className="px-4 py-3 text-right text-gray-500">${cpm.toFixed(2)}</td>
                      </tr>
                    );
                  })}
                </tbody>
                <tfoot>
                  <tr className="border-t-2 border-gray-200 bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right font-bold">{formatNumber(totals.impressions)}</td>
                    <td className="px-4 py-3 text-right font-bold">{formatCurrency(totals.revenue)}</td>
                    <td className="px-4 py-3 text-right font-bold">${avgCpm.toFixed(2)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
